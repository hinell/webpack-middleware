 ...
 * Usage: app.use(new Builder({compiler: webapckBundle}).middleware);
 */

var join  = require('path').join
  , mime  = require('mime');
    module.exports = Builder;
function Err (msg) { return {throw: function(){ throw msg }} };
function Builder (compiler,cfg) {
  ...
  this.compiler.plugin(['invalid','watch-run','run'],this.onFail_);
  this.compiler.compilers || (this.outputPath = this.compiler.outputPath);
  ...
  this.lazy // if lazy set then bundle
    && (this.build.state = true)
...
function DelayedResponses (build /*{state: bool}*/) {
  ...
  this.delay    = function (fn) { this.build.state && fn() || this.arr.push(fn) };
...
function PathExtractor(outputpath, publicpath, url) {
  ...
  this.join_        = function (a,b) { return a == "/" ?  "/"+b  :  (a||"")+"/"+b };
  // correct public path
  ...
  url.indexOf(publicpath) == 0
    && ((this.path = url.substr(publicpath.length), !!~this.path.indexOf("?"))
      && (this.path = this.path.split("?")[0]) // todo: check this out if not working well!
    // && (this.path = this.path.substr(0, this.path.indexOf("?")))
    , this.path = this.path && this.join_(outputpath,this.path) || outputpath
    )
};
...
function compilerRebuild () {
    ...
    && (  this.build.state = false
        , this.compiler.run(function (err) { if (err) { console.error('webpack-middleware: rebuilding'); throw err } }) )
    || (this.build.enforce = true);
};

// server listener, entry point if not in lazy mode by default here
function middleware (req,res,next) {
  ...
  this.lazy           // if we are in lazy mode
    && this.filename  // and have filename configured
  ...
  this.reqpath && this.responses.delay(function (reqpath) { // otherwise - delay requiest
    try {
    ...
    this.stat.isFile() && (function () {
      ...
      this.headers && this.headers.files
          ? (function () {
          ...
          : (function () {
            for (var header in this.headers) { res.setHeader(header,this.headers[header]) };
          }).call(this);
...
function onFail () {
  this.build.state && this.quiet || console.info('webpack-middleware: bundle is failed ');
  this.build.state = false;
